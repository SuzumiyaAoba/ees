/**
 * Updated embedding service using the new provider abstraction
 * Supports multiple providers (Ollama, OpenAI, Google AI) via the provider system
 */
...
export interface EmbeddingServiceV2 {
  ...
  readonly switchProvider: (providerType: string) => Effect.Effect<void, Error>

  readonly getCurrentProvider: () => Effect.Effect<string, never>
}

export const EmbeddingServiceV2 =
  Context.GenericTag<EmbeddingServiceV2>("EmbeddingServiceV2")

// Similarity calculation helper functions (same as original)
...
const make = Effect.gen(function* () {
  ...
  const switchProvider = (_providerType: string) =>
    Effect.gen(function* () {
      // This would require provider configuration lookup
      // For now, we'll delegate to the provider service
      return yield* Effect.fail(
        new Error(
          "Provider switching requires provider configuration - not implemented yet"
        )
      )
    })
...
export const EmbeddingServiceV2Live = Layer.effect(
  EmbeddingServiceV2,
  make
